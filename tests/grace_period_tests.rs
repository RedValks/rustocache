use rustocache::{RustoCache, CacheProvider, GetOrSetOptions, CacheError};
...
async fn test_grace_period_basic_functionality() {
    ...
    let result = cache.get_or_set(
        "test_key",
        || async { Ok(TestData { value: "original".to_string() }) },
        GetOrSetOptions {
    ...
    let result = cache.get_or_set(
        "test_key",
        || async { Ok(TestData { value: "refreshed".to_string() }) },
        GetOrSetOptions {
...
async fn test_grace_period_performance() {
    ...
    for i in 0..iterations {
        cache.get_or_set(
            &format!("key_{}", i),
            || async { Ok(TestData { value: format!("value_{}", i) }) },
            GetOrSetOptions {
    ...
    assert!(overhead_ratio < 1.5, "Grace period overhead too high: {:.2}x", overhead_ratio);
}
